...
;###############
 ...
 !Shift    		   = $FF03   ;;Amount we shifted each time
 !Width            = $FF04   ;;Width of the old tile
 !ShiftRight       = $FF05   ;;Shift right
 !ShiftLeft        = $FF06   ;;Shift left
...
LDX #$00             	;;Reset X
.loop
STZ $FF05,X          	;;Store zero
INX                   	;;Increment counter
CPX #$FB              	;;Check if everything's clean
...
LDA !A_Backup      ;;Load A backup
CMP #$7D           ;;Check if 0x7D
BNE CheckLineFlag          ;;If not, skip
...
;overflow
;==========================
SEP #$30           ;;8-bit accumulator and register
LDA #$18           ;;Select VRAM gate register
STA $804311        ;;Store
LDA #$09           ;;Select word increment
STA $804310        ;;Store
LDA #$80           ;;Amount to write to video port
STA $802115        ;;Store
REP #$20           ;;16-bit accumulator and register
LDA #$0400         ;;Load buffer position
LSR                ;;Divide by 2
STA $802116        ;;Store as VRAM address
LDA #$0C00        ;;Load amount to write
STA $804315        ;;Store (amount to write)
LDA #$FFFF         ;;Load RAM address
STA $804312        ;;Store
SEP #$20           ;;8-bit accumulator
LDA #$7F           ;;Load bank   
STA $804314        ;;Store
...
LDA $804212        ;;Wait for
BPL .loop          ;;V-Blank
LDA #$02           ;;Select DMA channel 1
STA $80420B        ;;Start transfer
PLB                ;;Restore bank
JMP $00E739                ;;Return
...
;==========================
LDA !TileMapPosition   ;;Load tilemap position
CMP #$00               ;;Check if zero
BNE WriteTilemap       ;;If not, skip
LDA #$40               ;;Load #$40 as initial position
STA !TileMapPosition   ;;Store in tilemap counter
...
LDX #$00
.loop
INC !TileMapPosition     	;;Increment tilemap position
LDA !TileMapPosition     	;;Load it
STA !TileMap,X    			;;Store in tilemap buffer
INX               			;;Increment index
LDA $6E           			;;Load palette byte
STA !TileMap,X    			;;Store in tilemap buffer 
INX               			;;Increment index
CPX !TileMapSize  			;;Check if tilemap fits the size
BNE .loop         			;;If it doesn't, loop
;;Bugfix
INX               ;;Increment index one last time
LDA $6E           ;;Load palette byte
STA !TileMap,X    ;;Store in tilemap buffer
...
LDA !BufferPosition  ;;Load buffer position
CLC                  ;;Prepare to add
ADC #$10             ;;Add 0x10
STA !BufferPosition  ;;Store in buffer position
...
LDY #$00                 ;;Reset Y
.loop
LDA !FontOffset,X        ;;Load current letter bytes
STA !NewTile,Y           ;;Store in new tile buffer
STA !RawTile,Y     ;;Store in untouched tile buffer 
INY                      ;;Increment counters
INX                      ;;Increment counters
CPY #$10                 ;;Check if we're done with all the 16 bytes
BNE .loop                ;;If not, loop
...
LDY #$00                   ;;Reset Y
.loop
LSR !NewTile,X   ;;Shift new tile byte left
INY              ;;Increment counter
CPY !ShiftRight  ;;Compare to shift amount
BNE .loop        ;;If not enough, loop
LDY #$00         ;;Reset Y
INX              ;;Increment byte counter
CPX #$10         ;;Check if we're done with all the 16 bytes
BNE .loop        ;;If not, loop
...
LDY #$00         ;;Reset Y
.loop
LDA !OldTile,Y   ;;Load old tile byte
ORA !NewTile,Y   ;;OR with equally positioned shifted letter byte
STA !OldTile,Y   ;;Store in old tile buffer
INY              ;;Increment new tile buffer index
CPY #$10         ;;Check if we're done with all the 16 bytes
BCC .loop        ;;If not, loop
...
LDX #$00               ;;Reset X
.loop
ASL !RawTile,X   ;;Shift original letter left
LDA !RawTile,X   ;;Load it
STA !NewTile,X         ;;Store in new tile buffer
INY                    ;;Increment shift count
CPY !ShiftLeft         ;;Compare to shift amount
BNE .loop              ;;If not enough, keep shifting
LDY #$00               ;;Reset Y
INX                    ;;Increment byte count
CPX #$10               ;;Compare to 16
BNE .loop              ;;If not enough, keep shifting
...
LDA !ShiftData,X      ;;Load tile width (using the index)
SEC                   ;;Prepare to Subtract
SBC !Shift    		;;Subtract amount we shited
STA !Width         ;;Store as old width
CMP #$00              ;;Check if zero
BEQ .Reset00          ;;If zero, reset everything
BMI .Reset08          ;;If less than zero, reset everything and activate "zero reached"
BPL GetWidth         ;;If greater than zero, continue normally
.Reset00
JSR ResetShift        ;;Redundancy save
JMP GetWidth     ;;Proceed
.Reset08              
JSR ResetShift        ;;Redundancy save
LDA #$00              ;;Load #$FF
SEC                   ;;Prepare to Subtract
SBC !Width         ;;Subtract old width
STA !Width      ;;Store in ZeroReached
LDA !Width           ;;Load ZeroReached
STA !ShiftLeft             ;;Store as left shift amount
LDA #$08                   ;;Load full space
SEC                        ;;Prepare to Subtract
SBC !Width           ;;Subtract ZeroReached
STA !ShiftRight            ;;Store as right shift amount
JMP LoadBackups     ;;Skip "SaveOldTile"

;==========================
GetWidth:
;==========================
LDA !Width       	;;Load old width
STA !ShiftRight     ;;Store as right shift amount
LDA #$08            ;;Load full space
SEC                 ;;Prepare to Subtract
SBC !Width       	;;Subtract old width
...
LDX #$00         ;;Reset X
.loop
LDA !NewTile,X   ;;Load new tile byte
STA !OldTile,X   ;;Store in old tile buffer
INX              ;;Increment counter
CPX #$10         ;;Check if we're done with all the 16 bytes
BNE .loop        ;;If not, loop
...
STZ !Shift
STZ !Width
DEC !LineFlag         ;;Activate line flag
...
LoadBackups2:
;==========================
SEP #$30        ;;8-bit register and accumulator
LDX !X_Backup   ;;Load X backup
LDY !Y_Backup   ;;Load Y backup
LDA !A_Backup   ;;Load A backup
PLB             ;;Restore bank
STA $0604,X     ;;Store as regular tilemap byte
JMP $00E70D
